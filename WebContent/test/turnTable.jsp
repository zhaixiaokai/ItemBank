...
<%@ page language="java" contentType="text/html; charset=utf-8"
	pageEncoding="utf-8"%>
<%@ page import="net.ib.common.*,java.util.*"%>
<%@ page import="net.ib.util.dao.*"%>
...
<%
	...
	List<Map> list	=	di.executeQuery("select count(*) from curse cu where special_field_id='11'");// 获取数据项数目 
	...
	String	sql="select * from (select rownum rn,cu.* from curse cu where special_field_id='11' order by rownum) where rn<="+rowPerPage*dipage+" and rn>"+(dipage-1)*rowPerPage;
...
<body>
	...
	<a id="bulkDel" href="javascript:void(0);"
		onclick="bulkDelete('check');">批量删除</a>
	<table width="100%" id="dataTable">
		<tr>
			<td><input type="checkbox" name="checkAll" id="checkAll"
				onclick="check_all(this,'check');">序号</td>
			<td>curse_id</td>
			<td>curse_name</td>
	...
	%>
		<tr id="tr<%=currentRow%>">
			<td><input type="checkbox" name="check"
				id="check<%=PageList.get(currentRow).get("curse_id")%>" /><%=PageList.get(currentRow).get("rn") %></td>
			...
			<td><a id="<%=PageList.get(currentRow).get("curse_id")%>"
				href="javascript:void(0)" onclick="funcDelet(this)">删除</a></td>
		</tr>
		<%
			}
	%>
    ...
    {
    %> <a href=turnTable.jsp?dipage=1>首页</a>&nbsp; <a
				href=turnTable.jsp?dipage= <%=(dipage-1) %>>&lt;上一页</a>&nbsp; <%
    }
	if(pageNum>5){
		if(dipage<=3){    	
    %> <a id="a_1" href=turnTable.jsp?dipage=1>1</a>&nbsp; <a id="a_2"
				href=turnTable.jsp?dipage=2>2</a>&nbsp; <a id="a_3"
				href=turnTable.jsp?dipage=3>3</a>&nbsp; <a id="a_4"
				href=turnTable.jsp?dipage=4>4</a>&nbsp; <a id="a_5"
				href=turnTable.jsp?dipage=5>5</a>&nbsp; ... <%
		}
    ...
    if(dipage==pageNum)//当前是末页
     ...
     {
	%> <a href=turnTable.jsp?dipage= <%=(dipage+1) %>>下一页&gt;</a>&nbsp; <a
				href=turnTable.jsp?dipage= <%=pageNum %>>末页</a>&nbsp; <%
    }     
    /* out.print("共"+pageNum+"条记录,共"+pageNum+"页，当前第"+dipage+"页，每页"+rowPerPage+"条记录，到第"); */
	%> 共<%=pageNum %>页&nbsp;&nbsp;&nbsp;到第<input id="input_refferToNum"
				type="text" size="3">页&nbsp;&nbsp;&nbsp <a id="sure"
				href="javascript:void(0);" onclick="refferTo(<%=pageNum%>);">确定</a>
			</td>
		</tr>
	</table>
