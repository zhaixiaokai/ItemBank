...
<script type="text/javascript">
	...
	//生成NestedTable表格，并对于已经属于所选的成员类型的权限的复选框打钩
	function CreateTree(){
		//将原来的生成的div内容进行清空
		$("#TableDiv").empty();
		$("#button").empty();
		...
		//对于已经属于该用户类型的的权限将其复选框勾选;
		var PresentAuthorityUserType;
		$.ajax( {
			type : "post",
			dataType : "text",
			async : false,
			url : "PresentUserTypeAuthorityGetAction",
			data:{"MemberType":MemberType},
			success : function(result) {
				
				PresentAuthorityUserType=result;//角色目前拥有的权限
				var PresentAuthorityUserTypeIds=PresentAuthorityUserType.split(",");
				...
				for(var k=0;k < CheckBoxList.length; k++){
					...
					//alert(TempId);
					for(var i=0;i<PresentAuthorityUserTypeIds.length;i++){
						
						if(TempId==PresentAuthorityUserTypeIds[i]){
	...
	function Save() {
		...
		for ( var i = 0; i < CheckBoxList.length; i++) {
			...
			// 没有选中的checkbox表示这些功能不属于该用户类型，若这些权限存在于数据表中则从中删除
			 else {
					if (DeleteCount == 0) {
		...
		if (confirm("确定添加？")) {
			$.ajax({
				...
				success : function(result) {
					alert(result.text);
					document.location.href = "UserMemberAuthority.jsp";
				}
			});
...
<body class=" yui-skin-sam">
	...
	<div id="act_top">
		<a href="../functionList/HomePage.html">首页</a>&nbsp;&gt;&gt;&nbsp;<a
			...
			href="javascript:void(0)">按成员授权</a>
	</div>
	...
	<table height="50px" width="100%">
		<tr>
			<td width="180px">成员类型：<select id="membertype" name="membertype">
