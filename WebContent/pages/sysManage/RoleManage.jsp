...
function CreateDataTableForRole(){
	(function () {
	    ...
	    var myDataSource = new YAHOO.util.DataSource("RoleDataGetAction?timestamp="+timestamp+"&");  
	    ...
	    myDataSource.responseSchema = {  
           resultsList: "records",
           fields: ["role_id","name","explain","rn"],
           metaFields: {
        ...
        YAHOO.widget.DataTable.Formatter.Delete = function(elCell, oRecord, oColumn, oData){
            var Id=oRecord.getData("role_id");
        ...
        YAHOO.widget.DataTable.Formatter.checkbox = function(elCell, oRecord, oColumn, oData){
            ...
            elCell.innerHTML = "<input name=\"CheckBoxs\" id = \"" + oRecord.getData('role_id')
...
function DeleteRow(o){
	...
	if(confirm("确定要删除该记录？")){
		$.ajax({
			...
			dataType : "json",
			async : false,
			url : "RoleDeleteAction?DeleteId="+id,
			success : function(result) {
				alert(result.text);
				//删除完成后重新载入数据表~
				CreateDataTableForRole();
			}
		});
	}
	else{
...
function ModifyRow(Id,Name,Explain){
    ...
    document.getElementById("RoleExplain").value=Explain.replace(/n1/g,"\n").replace(/n2/g,"n");
    document.getElementById("OldRoleId").value=Id;
    //根据客户端浏览器高度设置半透明罩的高度 
...
function	CheckValidation(){
	var	RoleName	= document.modifyForm.RoleName.value;
	var	RoleId	= document.modifyForm.RoleId.value;
	var	RoleExplain=document.modifyForm.RoleExplain.value;
	if(RoleId==""){
		alert("请填写角色唯一标识符");
		return;
	}
	...
	<div id="act_content2">
		...
		<div id="container">
			<div id="panel1" style="z-index: 1999">
				<div class="hd" style="z-index: 2000">修改角色信息</div>
				<div class="bd" style="z-index: 2000">
					<form name="modifyForm" action="RoleUpdateAction"
						id="modifyForm">
						<input type="hidden" name="OldRoleId" id="OldRoleId" />
						<table>
							<tr>
								<td width="150px">角色唯一标识符：</td>
							...
							<tr>
								<td width="100px">角色说明：</td>
								<td><textarea id="RoleExplain" name="RoleExplain"></textarea></td>
							</tr>
						</table>
						<div align="center">
