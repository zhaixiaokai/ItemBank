...
function DeleteRow(o){
	...
	if(confirm("确定要删除该记录？")){
		$.ajax({
			...
			dataType : "json",
			async : false,
			url : "KnowledgePointDeleteAction?DeleteId="+id,
			success : function(result) {
				alert(result.result);
				//删除完成后重新载入数据表~
				CreateKnowledgePointTableByCurseId(SpecialFieldId);
...
function BulkDelete(){
	...
	if(confirm("确定要删除？")){
		$.ajax( {
			...
			dataType : "json",
			async : false,
			url : "KnowledgePointBulkDeleteAction",
			data:"DeleteId="+param,
			success : function(result) {
				alert(result.text);
				CreateKnowledgePointTableByCurseId(SpecialFieldId);
...
function	CheckValidation(){
	...
	var	explain	=	document.getElementById("KnowledgeContent").value;
	if(name==""){
		alert("知识点名称不能为空");
		modifyForm.KnowledgePointName.focus();
		return false;
	}
	if(!CheckIfChinaNumbLetter(name)){
		alert("知识点名称只能由汉字、数字、字母构成");
		modifyForm.KnowledgePointName.focus();
		return false;
	}
	...
	if(id==""){
		alert("知识点编号不能为空");
		modifyForm.KnowledgePointId.focus();
		return false;
	}
...
<body class=" yui-skin-sam">
	...
	<!-- 右侧界面 -->
	<div id="act_top">
		<a target="mainFrame" href="../functionList/teachingManage.jsp">教学管理</a>&nbsp;&gt;&gt;&nbsp;知识点管理<a
			href="knowledgePointManage.jsp">&nbsp;&gt;&gt;&nbsp;管理知识点</a>
	</div>
					...
					<form name="modifyForm" action="KnowledgePointUpdateAction"
						...
						<table>
							...
							<tr>
								...
								<td><textarea id="KnowledgeContent" name="KnowledgeContent" cols="80" rows="5"></textarea></td>
							</tr>
						</table>
						<div align="center">
