...
//		(ChapterId);
	    ...
	    $.ajax( {
			...
			dataType : "json",
			async : false,
			url : "KnowledgeListForCheckGetByTeachingMaterialIdAction",
			...
			success : function(result) {
				...
				{
					if(result.data[t].chapter_id==ChapterId) {
...
//		alert(ChapterId);
	    ...
	    if(confirm("确定要保存修改？")){
			$.ajax( {
				...
				dataType : "json",
				async : false,
				url : "SaveKnowledgePointConfigByChapterAction?selected_point="+selected_point+"&MyChapterId="+ChapterId,
				success : function(result) {
					alert(result.result);
