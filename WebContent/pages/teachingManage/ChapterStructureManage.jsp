...
<script type="text/javascript" src="../../servJs/TeachingChapterTreeOperate.js"
	charset="GBK"></script>
...
//	alert(CourseId);
	...
	function GetTeacherDepTree() {
		...
		$.ajax({
			...
			dataType : "json",
			async : false,
			url : "FunctionGetChapterTreeDataAction",
			data : "MyTeachingMaterialId="+TeachingMaterialId,
	...
	function ConfirmModify(){
		...
		if(newText==""||newText==null){
			alert("修改失败：请填写章节节点名称！");
			document.modifyForm.NewTeacherDepartmentText.focus();
		}
		else if(newText.length>20){
			alert("修改失败，章节节点名称的长度不能超过20");
			document.modifyForm.NewTeacherDepartmentText.focus();		
		}
		else if(!CheckIfChinaNumbLetter(newText)){
			alert("修改失败，章节节点名称只能由汉字、数字、字母组成");
			document.modifyForm.NewTeacherDepartmentText.focus();
		}
		else if(newExplain.length>500){
			alert("修改失败，章节节点说明的长度不能超过500");
			document.modifyForm.NewTeacherDepartmentText.focus();
		}
		else{
			if(confirm("确认修改？")){
	    		...
	    		$.ajax( {
	    			...
	    			data:"nodeid="+currentNode.data.id+"&node_explain="+newExplain+"&text="+newText+"&table="+"SYS_TEACHING_CHAPTER_TREE"+"&treeId="+currentNode.data.tree_id,
	    			success : function(result) {
	    				alert(result);
	    			},
	    			error:function(){
	...
	function ConfirmAddChild(){
   	...
   	function	getModifyNodeInfo(){
   	    ...
   	    document.getElementById("PannelHeader").innerHTML="修改教学章节结构信息";
   	...
   	function	getAddPreviousNodeInfo(){
   	    ...
   	    document.getElementById('fade').style.height=document.body.clientHeight+"px" ;
   	    document.getElementById('fade').style.display='block';
   	    document.getElementById("PannelHeader").innerHTML="添加前置章节节点";
   	...
   	function	getAddAfterNodeInfo(){
   	    ...
   	    document.getElementById('fade').style.height=document.body.clientHeight+"px" ;
   	    document.getElementById('fade').style.display='block';
   	    document.getElementById("PannelHeader").innerHTML="添加后置章节节点";
   	...
   	function	getAddChildNodeInfo(){
   	    ...
   	    document.getElementById('fade').style.height=document.body.clientHeight+"px" ;
   	    document.getElementById('fade').style.display='block';
   	    document.getElementById("PannelHeader").innerHTML="添加子章节节点";
...
<body class=" yui-skin-sam">
	...
	<!-- 右侧界面 -->
	<div id="act_top">
		<a href="javascript:void(0)">教学管理</a>&nbsp;&gt;&gt;&nbsp;<a
			href="javascript:void(0)">教材管理</a>&nbsp;&gt;&gt;&nbsp;<a
			href="javascript:void(0)">章节结构管理</a>
	</div>
	...
	<div id="act_content2">
		<div id="te1" style="width: 100%; height: 80px;">
			<a href="javascript:void(0)" onclick="addPreviousNode()">添加前置章节节点</a> <strong>|</strong>
			<a href="javascript:void(0)" onclick="addAfterNode()">添加后置章节节点</a><strong>|</strong>
			<a href="javascript:void(0)" onclick="modifyCurrentNode()">修改章节节点</a> <strong>|</strong>
			<a href="javascript:void(0)" onclick="deleteCurrentNode()">删除章节节点</a> <strong>|</strong>
			<a href="javascript:void(0)" onclick="cutCurrentNode()">剪切章节节点</a> <strong>|</strong>
			...
			<strong>|</strong> <a href="javascript:void(0)"
				...
				href="javascript:void(0)" onclick="PasteAsChildNode()">粘帖为子章节节点</a><strong>|</strong>
			<a href="javascript:void(0)" onclick="addChildeNode()">添加子章节节点</a>
