...
//数组去重
	...
	function get_authorized_itembank(){
		...
		dataType : "json",
		async : false,
		url : "GetAuthorizedItembankByuserAction?member="+member+"&number="+number,
		success : function(result) {
			result = eval(result);
	...
	function SaveConfig(){
		...
		$.ajax( {
			...
			dataType : "json",
			async : false,
			url : "SaveAuthorityConfigByuserSelectcourseAction?authorized_es="+authorized_es+"&number="+number+"&member="+member+"&courseid="+course_id+"&majorid="+major_id,
	...
	$.ajax( {
		...
		dataType : "json",
		async : false,
		url : "SaveAuthorityConfigByuserSelectnonecourseAction?authorized_es="+authorized_es+"&number="+number+"&member="+member+"&majorid="+major_id,
		success : function(responseText, statusText) {
			alert(responseText.message);
			document.location.href = "authority_user.jsp";
		},
	...
	$.ajax( {
		...
		dataType : "json",
		async : false,
		url : "SaveAuthorityConfigByuserAction?authorized_es="+authorized_es+"&number="+number+"&member="+member,
		success : function(responseText, statusText) {
			alert(responseText.message);
			document.location.href = "authority_user.jsp";
		},
...
<body><jsp:include page="../frame/Frame1.jsp"></jsp:include>
	...
	<fieldset>
		<legend>目标用户选择</legend>
		<table style="font-size: 12px" class="tableheight">
			<tr>
					<td nowrap>成员类型： <select id="selectmembertype" name="selectmembertype" style="font-size: 12px">
				...
				<td><input type="text" size=8  id="usernumber" name="usernumber">
				</td>
