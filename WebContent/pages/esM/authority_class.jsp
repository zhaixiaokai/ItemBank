...
<link rel="stylesheet" type="text/css" href="../css/MainFrame.css" /><!-- 加上会有右侧滚动条痕迹~
		...
		success : function(result) {
			...
			for(var	i=0;i<result.data.length;i++){
				$("#SelectCurse").append("<option id=\""+result.data[i].curse_id+
						"\" value=\""+result.data[i].curse_id+"\" onclick=\"GetTeachingClass('"+result.data[i].curse_id+"')\">"+
...
function GetTeachingClass(id){
	...
	$.ajax( {
		...
		dataType : "json",
		async : false,
		url : "GetTeachingClassAction?courseid="+id,
		success : function(result) {
			$("#SelectTeachingClass ").empty();
			$("#SelectTeachingClass").append("<option value=\"\" id=\"selectnoneclass\">--请选择--</option>");
			for(var	i=0;i<result.length;i++){
				$("#SelectTeachingClass").append("<option id=\""+result[i].curse_class_id+
						"\" value=\""+result[i].class_name+"\" >"+
						result[i].class_name+"</option>");
			}
		},
		error:function(){
		}
...
//查询部分 ，组织机构列表
var ReferSchoolStructure=null;
function	ReferGetSchoolStructureList(){
	$.ajax( {
		type : "post",
		dataType : "json",
		async : false,
		url : "ReferSchoolStructureOptionsGetAction",
		success : function(result) {
			ReferSchoolStructure=result.data;
		},
		error:function(){
		}
	});
...
//查询部分获取专业
function RefergetMajor(o){
	document.getElementById("Refer_College").innerHTML=o;
}

//查询部分获取专业id
var refer_major_id=null;
function	ReferGetCourseBySpecialfield(){
	RefergetMajor(this.innerHTML);
	RefergetCurse(this);
	refer_major_id=this.id;
}

//查询部分显示课程列表
function RefergetCurse(o){
	var	SelectedSpecialField	=	o.id;
	$.ajax( {
		type : "post",
		dataType : "json",
		async : false,
		url : "ReferCurseSelectBySpecialFieldAction",
		data:"specialFieldId="+SelectedSpecialField,
		success : function(result) {
			$("#ReferSelectCurse ").empty();
			$("#ReferSelectCurse").append("<option value=\"\" id=\"selectnonerefercurse\">--请选择--</option>");
			for(var	i=0;i<result.data.length;i++){
				$("#ReferSelectCurse").append("<option id=\"refer"+result.data[i].curse_id+
...
//数组去重
	...
	function get_authorized_itembank(){
		...
		//获取教学班级
		var options = document.getElementById("SelectTeachingClass").options;
		var index = document.getElementById("SelectTeachingClass").selectedIndex;
		var selectedOption = options[index];
		teachingclass = selectedOption.id;
		//判断是否选择教学班级
		if (teachingclass == '' || teachingclass == 'selectnoneclass'||teachingclass==null) {
			alert("请选择教学班级");
			return 1;
		} 
		//查询该角色成员的权限
		...
		dataType : "json",
		async : false,
		url : "GetAuthorizedItembankByteachingclassAction?teachingclass="+teachingclass,
		success : function(result) {
			result = eval(result);
	...
	function showTable() {
		var options = document.getElementById("ReferSelectCurse").options;
		var index = document.getElementById("ReferSelectCurse").selectedIndex;
		var selectedOption = options[index];
		...
		//var itembanklistdata = null;
		//若没有选择课程，查找相应试题库
		if (selectedOption.id == '' || selectedOption.id == 'selectnonerefercurse'||selectedOption.id==null) {
			var refermajorid=null;
			//去除id前refer字段
			refermajorid=refer_major_id.substring(5, refer_major_id.length);
			$.ajax({
				type : "post",
				dataType : "json",
				async : false,
				url : "GetItembankBymajorAction?majorid=" + refermajorid,
				success : function(result) {
					result = eval(result);
			...
			//获取课程名称
			//去除id前refer字段
			refercourseid=selectedOption.id.substring(5, selectedOption.id.length);
			...
			$.ajax( {
				...
				dataType : "json",
				async : false,
				url : "GetCurseNameBycurseAction?curseid="+refercourseid,
				success : function(result) {
		...
		//获取试题库信息
			...
			$.ajax( {
				...
				dataType : "json",
				async : false,
				url : "GetItembankBycurseAction?curseid="+refercourseid,
				success : function(result) {
					result = eval(result);
					itembanklistdata = result;
				},
	...
	function SaveConfig(){
		//判断是否选择课程
		//若选择课程
		if (course_id != '' &&course_id != 'selectnonerefercurse') {
		//获取所有的Checkbox
		var authorized_es=[];
		...
		//保存配置
		real_course_id=course_id.substring(5, course_id.length);
		$.ajax( {
			type : "post",
			dataType : "json",
			async : false,
			url : "SaveAuthorityConfigByteachingclassSelectclassAction?authorized_es="+authorized_es+"&teachingclass="+teachingclass+"&courseid="+real_course_id+"&majorid="+refer_major_id,
			success : function(responseText, statusText) {
				alert(responseText.message);
				document.location.href = "authority_class.jsp";
			},
			error:function(){
			}
		});
	...
	$.ajax( {
		...
		dataType : "json",
		async : false,
		url : "SaveAuthorityConfigByteachingclassSelectnoneclassAction?authorized_es="+authorized_es+"&teachingclass="+teachingclass+"&majorid="+refer_major_id,
		success : function(responseText, statusText) {
			alert(responseText.message);
			document.location.href = "authority_class.jsp";
		},
	...
	$.ajax( {
		...
		dataType : "json",
		async : false,
		url : "SaveAuthorityConfigByteachingclassAction?authorized_es="+authorized_es+"&teachingclass="+teachingclass,
		success : function(responseText, statusText) {
			alert(responseText.message);
			document.location.href = "authority_class.jsp";
		},
...
<body><jsp:include page="../frame/Frame1.jsp"></jsp:include>
	...
	<fieldset>
		...
		<tr>
				...
				<td nowrap>教学班： </td>
				<td><select id="SelectTeachingClass"
					style="font-size: 12px"><option selected="selected">--请选择--</option>
				</select></td>
			</tr>
		</table>
	...
	<fieldset>
		...
		<tr>
				...
				<td  nowrap>专业：</td>
				<td id="SelectBasic"><div id="Refer_SpecialField"></div></td>
				<td nowrap>课程：</td>
				<td><select id="ReferSelectCurse"
					style="font-size: 12px"><option selected="selected">--请选择--</option>
				</select></td>
	...
	ReferGetSchoolStructureList();
	loadMenu("Refer_SpecialField",ReferSchoolStructure,ReferGetCourseBySpecialfield,"Refer_College");
</script>
