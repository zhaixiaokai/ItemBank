...
function getTM(){
	...
	$.ajax( {
		...
		dataType : "json",
		async : false,
		url : "TeachMaterialGetByCourseIdAction",
		data:"SelectCurse="+SelectedCurseField,
		success : function(result) {
			$("#SelectTM ").empty();
			$("#SelectTM").append("<option value=\"\">--请选择--</option>");
			for(var	i=0;i<result.data.length;i++){
				$("#SelectTM").append("<option id=\""+result.data[i].teaching_material_id+
						"\" value=\""+result.data[i].teaching_material_id+"\">"+
...
<body class=" yui-skin-sam"> 
	...
	<table width="100%" class="CContent">
		...
		<tr>
			<td width="100%">
			<form id="myForm" action="automOnChapter1.jsp">
				<table width="100%">
							...
							<fieldset style="height: 100%;">
								...
								<table >
									<tr >
										...
										<td width="50px">课程:</td>
										<td width="100px">
											<select id="SelectCurse" name="SelectCurse" style="font-size: 12px"><option value="" selected="selected">--请选择--</option>
											</select>
										</td>
										<td>试题用途:</td>
										<td><select name="SelectIBUseage" id="SelectPaperUseage"></select></td>	
										<td><input type="button" class="button" value="查询" onclick="IBQurey()"/></td>
...
<!-- 											<select name="SelectDifficulty" id="SelectDifficulty">
											...
											<input type="text" id="SelectDifficulty" name="SelectDifficulty">
										</td>
															
										<td width="70px">目标试卷库:</td>
										<td width="100px">
											<select id="SelectEPDB" name="SelectEPDB"><option value="" selected="selected">--请选择--</option>
											</select>
										</td><td>选择教材：											
											<select id="SelectTM" name="SelectTM"><option value="" selected="selected">--请选择--</option>
											</select></td>
									</tr>
									<tr height="50px">
										<td>试卷总分:</td>
										<td><input name="totalScore" type="text"
											id="totalScore" />(分)</td>
										<td>考试时长:</td>
										<td ><input name="totalTime" type="text"
											id="totalTime" />（分钟）</td>
