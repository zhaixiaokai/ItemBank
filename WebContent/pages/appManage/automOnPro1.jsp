...
<script>
var	CourseId='<%=request.getParameter("SelectCurse")%>';  
//var	CourseId='020000102';
...
function	KnowledgeList(){
	...
	$.ajax( {
		...
		success : function(result) {
			//alert(document.getElementById("TD_"+result.data[i].progress_teaching_node_id));
			GetKnowledgeList=result.data;
			//alert(GetKnowledgeList);
			 for(var i=0;i<result.data.length;i++){
				$("#TD_"+result.data[i].progress_teaching_node_id).empty();
			} 
			for(var	i=0;i<result.data.length;i++){
					document.getElementById("TD_"+result.data[i].progress_teaching_node_id).innerHTML+="<br>"
