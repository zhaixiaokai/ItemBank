	...
	$.ajax( {
		...
		success : function(result) {
			...
			for(var	i=0;i<dataJson.length;i++){
				...
				innerStr+='<td style="border: 0px"><input type="radio" value="'+dataJson[i].epdb_id+'" name="radio">'+
				dataJson[i].epdb_name+'</td>';
				if((i+1)%5==0){
					innerStr+='</tr><tr>';
				}
			}
...
<body>
	...
	<div style="margin-left: 25px; margin-top: 10px">
		<form action="addPaperForm.jsp" id="IBSelectForm">
...
<!-- 			<table>
	...
	<script>
		...
		//加载用途选项,生成显示分块框架 每个显示试卷库列表的表格中<td>的id为"TD_"+字典项值 ，在查询后根据此ID填充对应框架中的内容
		$.ajax( {
			type : "post",
			dataType : "json",
			async : false,
			...
			success : function(result) {
				...
				for(var i=0,j=result.data.length;i<j;i++){
					...
					$("#div").append('<table width="100%" border="0" cellspacing="0" cellpadding="0"class="tableList">'+
							'<tr align="center"><td class="tr1">试卷库列表('+result.data[i].name+')</td></tr>'+
