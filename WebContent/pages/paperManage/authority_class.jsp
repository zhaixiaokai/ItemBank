...
//数组去重
	...
	function get_authorized_epdb(){
		...
		dataType : "json",
		async : false,
		url : "GetAuthorizedEPDBByteachingclassAction?teachingclass="+teachingclass,
		success : function(result) {
			result = eval(result);
	...
	function SaveConfig(){
		...
		$.ajax( {
			...
			dataType : "json",
			async : false,
			url : "SaveEPDBAuthorityConfigByteachingclassSelectcourseAction?authorized_epdb="+authorized_epdb+"&teachingclass="+teachingclass+"&courseid="+real_course_id+"&majorid="+refer_major_id,
		...
		$.ajax( {
			...
			dataType : "json",
			async : false,
			url : "SaveEPDBAuthorityConfigByteachingclassSelectnonecourseAction?authorized_epdb="+authorized_epdb+"&teachingclass="+teachingclass+"&majorid="+refer_major_id,
			success : function(responseText, statusText) {
				alert(responseText.message);
				document.location.href = "authority_class.jsp";
			},
	...
	function showTable() {
		...
		if (selectedOption.id == '' || selectedOption.id == 'selectnonerefercurse'||selectedOption.id==null) {
			...
			$.ajax({
				...
				dataType : "json",
				async : false,
				url : "GetEPDBBymajorAction?majorid=" + refermajorid,
				success : function(result) {
					result = eval(result);
					epdblistdata = result;
					//计算课程数，存储对应课程id
		...
		//获取试题库信息
			...
			$.ajax( {
				...
				dataType : "json",
				async : false,
				url : "GetEPDBBycurseAction?curseid="+refercourseid,
				success : function(result) {
					result = eval(result);
					epdblistdata = result;
				},
		...
		<fieldset>
			...
			<table>
				<tr>
					...
					<td nowrap>教学班：</td>
					<td><select id="SelectTeachingClass" style="font-size: 12px"><option
								selected="selected">--请选择--</option>
					</select></td>
				</tr>
			</table>
		...
		<fieldset>
			...
			<table style="font-size: 12px" class="tableheight">
				<tr>
					...
					<td nowrap>课程：</td>
					<td><select id="ReferSelectCurse" style="font-size: 12px"><option
								selected="selected">--请选择--</option>
					</select></td>
