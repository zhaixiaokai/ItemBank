...
function FunctionFullfil() {
	...
	refersortleaf=this.id;//试卷库查询分类id获取
}
...
function getusedata() {
	...
	/* function sortSelect() {
		self.parent.frames["mainFrame"].location = "paperManage.jsp";
	} */
...
function	CreateDataTableForSort(){
	(function () {
	    ...
	    var myDataSource = new YAHOO.util.DataSource("EPDBManageAction?"+"ts="+(new Date()).valueOf()+"&");  
	    ...
	    YAHOO.widget.DataTable.Formatter.Delete = function(elCell, oRecord, oColumn, oData){
            ...
            elCell.innerHTML=" <a href='#' onclick=ModifyRow('"+id+"','"+name+"','"+use+"','"+explain+"')>详细</a>";
	    }  
...
function refer(){
	...
	(function () {
		var History = YAHOO.util.History,
		 ...
		 var myDataSource = new YAHOO.util.DataSource("ReferEPDBAction?refer_leafnode="+leafnode+"&referuse="+referyuse+"&if_under_defaultsort="+if_under_defaultsort+"&ts="+(new Date()).valueOf()+"&");  
	    myDataSource.responseType = YAHOO.util.DataSource.TYPE_JSON;  
	    /** 
	     * 设置Fields以摘出页面上要用到的数据列，这里的列不一定全部会被显示在页面上。决定哪些列显示的语句在下面。 
	     */  
 ...
 -->			</tr>
		...
		<div id="container">
			<div id="panel1" style="z-index: 1999">
				<div class="hd" style="z-index: 2000">查看试卷库</div>
				<div class="bd" style="z-index: 2000">
					<form name="modifyForm" action="SortModifyAction" id="modifyForm">
						...
						<table>
							...
							<tr>
								<td width="150px" nowrap align="right">试卷库标识符：</td>
