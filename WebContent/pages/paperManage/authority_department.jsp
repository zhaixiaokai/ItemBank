...
function get_authorized_epdb(){
	...
	dataType : "json",
	async : false,
	url : "GetAuthorizedEPDBBydepartmentAction?member="+member+"&departmentid="+department_id+"&uppernode="+UpperNode,
	success : function(result) {
		result = eval(result);
...
function SetAuthorized(){
	...
	for(var	a=0;a<authorized_epdb.length;a++){
		var authorized_epdb_id=authorized_epdb[a].epdb_id;//已授权的试卷库id
...
function SaveConfig(){
	...
	$.ajax( {
		...
		dataType : "json",
		async : false,
		url : "SaveEPDBAuthorityConfigBydepartmentSelectcourseAction?authorized_epdb="+authorized_epdb+"&departmentid="+department_id+"&member="+member+"&courseid="+course_id+"&majorid="+major_id,
		...
		$.ajax( {
			...
			dataType : "json",
			async : false,
			url : "SaveEPDBAuthorityConfigBydepartmentSelectnonecourseAction?authorized_epdb="+authorized_epdb+"&departmentid="+department_id+"&member="+member+"&majorid="+major_id,
			success : function(responseText, statusText) {
				alert(responseText.message);
				document.location.href = "authority_department.jsp";
			},
...
<body><jsp:include page="../frame/Frame1.jsp"></jsp:include>
	...
	<div class="div_style2">
		<fieldset>
			<legend>目标成员类型</legend>
			<table>
				<tr>
					<td nowrap>成员： <select id="selectmembertype"
						name="selectmembertype" style="font-size: 12px">
							<option selected="selected" id="selectnonetype"
								onclick="getSchoolStructureList('selectnonetype')">-请选择-</option>
